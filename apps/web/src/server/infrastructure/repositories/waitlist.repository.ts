import { DatabaseError, type JoinWaitlist } from '@riposte/core'
...
export class WaitlistRepository implements IWaitlistRepository {
  ...
  async add(command: JoinWaitlist): Promise<Result<{ alreadyExists: boolean }, DatabaseError>> {
      ...
      try: async () => {
        const inserted = await this.db
          ...
          .values({
            ...
            usesStripe: command.usesStripe,
            hasDisputes: command.hasDisputes,
          })
          .onConflictDoNothing()
          .returning({ id: waitlist.id })
        return { alreadyExists: inserted.length === 0 }
      },
      catch: (cause) => new DatabaseError({ message: 'Failed to add to waitlist', cause }),
    })
  }
}
