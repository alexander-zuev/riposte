/**
 * @riposte/core/client - Browser-safe entry point
 *
 * Excludes server-only code.
 * Safe for bundling in frontend applications.
 *
 * Exports:
 * - domain/         Business concepts, primitives
 * - errors/         Error classes (client-safe)
 * - logger/         Console logger (client-safe)
 */

export * from './domain/messaging'
export * from './domain/primitives'
export * from './domain/auth'
export * from './domain/chat'
export * from './domain/disputes/dispute-case-message.commands'
export * from './domain/disputes/dispute-case-message.dto'
export * from './domain/disputes/dispute-case-message.queries'
export * from './domain/disputes/dispute.dto'
export * from './domain/disputes/dispute-workflow-policy'
export * from './domain/disputes/dispute.queries'
export * from './domain/disputes/stripe-dispute-taxonomy'
export * from './domain/dispute-playbooks/dispute-playbook.schemas'
export * from './domain/products'
export * from './domain/waitlist'
export * from './infrastructure/errors/application.errors'
export * from './infrastructure/errors/domain.errors'
export * from './infrastructure/logger'
export * from './api/routes'
export * from './rpc/result-wire'
